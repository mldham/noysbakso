...
if ($kurir === "Gojek") {
    $biayaKurir = 12000;
    $totalBayar = $totalMenu + 12000;
} if ($kurir === "Grab") {
    $biayaKurir = 14000;
    $totalBayar = $totalMenu + 14000;
} if ($kurir === "Shopee") {
    $biayaKurir = 10000;
    $totalBayar = $totalMenu + 10000;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($selected_products)) {
        ...
        $stmt->bind_param("ississ", $user_id, $alamat, $kurir, $biayaKurir, $payment, $totalBayar);
        ...
        foreach ($selected_products as $product_id) {
            ...
            $query = "INSERT INTO detail_pesanan (pesanan_id, no_produk, jumlah, harga, total_harga) VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("iisss", $last_id, $product_id, $jumlah, $harga, $totalMenu);
            if ($stmt->execute()) {
                echo "Query berhasil dieksekusi.";
            } else {
                echo "Terjadi kesalahan saat menyimpan data pesanan ke dalam database: " . $stmt->error;
