...
session_start();
include 'connect.php';
$query = "SELECT * FROM pemesanan ORDER BY pesanan_id DESC LIMIT 1";
...
if ($stmt->execute()) {
  ...
  $query = "SELECT * FROM detail_pesanan WHERE pesanan_id = " . $pesanan['pesanan_id'];
  $stmt = $conn->prepare($query);
  ...
  $totalMenu = $totalBayar - $biayaKurir;
